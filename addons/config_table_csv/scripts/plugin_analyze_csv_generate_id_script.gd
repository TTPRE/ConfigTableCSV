...
static func generate_id_script(data: Array[PackedStringArray], csv_dir_path: String, csv_file_name_without_suffix: String) -> void:
	...
	var id_script_path : String = csv_dir_path + csv_file_name_without_suffix + PluginCTSConfigHelper.ID_SCRIPT_NAME_SUFFIX
	
	var id_script_file : FileAccess = FileAccess.open(id_script_path, FileAccess.WRITE)
	
	id_script_file.store_line(PluginCTSConfigHelper.NEW_FILE_ANNOTATION)
	
	var id_class_name : String = PluginCTSConfigHelper.get_class_name(csv_file_name_without_suffix, PluginCTSConfigHelper.ID_CLASS_NAME_SUFFIX)
